            ...
            <span className="flex gap-6 items-center justify-center text-white">
                ...
                {isUserAuthenticated ?
                    <span className="relative font-lato">
                        <PiUserCircleLight className="text-4xl my-[1px] cursor-pointer" onClick={() => setProfilePopUp(!profilePopUp)} />
                        {profilePopUp && <div className={`absolute flex flex-col gap-4 custom_shadow py-4 px-6 -right-6 rounded-lg ${currentTheme === 'light' ? "bg-white" : "bg-gray"}`}>
                            <Link to="/profile" className={`px-6 py-[6px] text-black bg-white rounded-md text-lg ${currentTheme === 'light' && "border-2 border-blue_100"}`}>Profile</Link>
                            <button className="px-6 py-[6px] text-white rounded-md text-lg bg-red" onClick={logoutUser}>Logout</button>
                        </div>}
                    </span>
